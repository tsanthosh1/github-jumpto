...
function generateEntries(org, repos = []) {
    ...
    repos.forEach(repo => {
        ...
        entries.push(
            ...
            { searchTerm: [`${fullRepo}/pulls`, `${fullRepo}/pr`, `${fullRepo}/pull request`], url: `/${fullRepo}/pulls`, displayName: `${fullRepo} -> PR` },
            ...
            { searchTerm: [`${fullRepo}/issues/search`], url: `/${fullRepo}/issues?q=%s`, displayName: `${fullRepo} -> Issues Search`, searchPlaceholder: "Search Issues", displayTemplate: `${fullRepo} -> Search Issues for "%s"` },
